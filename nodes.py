...
class BespokeAI3DGeneration:
    ...
    RETURN_NAMES = ("glb_path", "obj_path", "model_url", "enhanced_image_url", "credits_used")
    ...
    def generate_3d(self, image, api_key, resolution, with_texture, ai_enhancement,
        ...
        if glb_url:
            ...
            glb_path = self.download_file(glb_url, f"model_{timestamp}.glb")
            print(f"[BespokeAI] GLB saved: {glb_path}")
        
        if obj_url:
            print("[BespokeAI] Downloading OBJ file...")
            obj_path = self.download_file(obj_url, f"model_{timestamp}.obj")
            print(f"[BespokeAI] OBJ saved: {obj_path}")
        
        print("[BespokeAI] 3D generation complete!")
        
        return (glb_path, obj_path, model_url, enhanced_image_url, credits_used)
...
class BespokeAI3DGenerationFromURL:
    ...
    def generate_3d(self, image_url, api_key, resolution, with_texture, ai_enhancement,
        ...
        if glb_url:
            print("[BespokeAI] Downloading GLB file...")
            glb_path = self._main.download_file(glb_url, f"model_{timestamp}.glb")
            print(f"[BespokeAI] GLB saved: {glb_path}")
        
        if obj_url:
            print("[BespokeAI] Downloading OBJ file...")
            obj_path = self._main.download_file(obj_url, f"model_{timestamp}.obj")
            print(f"[BespokeAI] OBJ saved: {obj_path}")
